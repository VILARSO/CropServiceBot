...
# Налаштування логування (якщо ще нема)
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Отримуємо токен бота та URI MongoDB з оточення
BOT_TOKEN = os.getenv("BOT_TOKEN")
MONGODB_URI = os.getenv("MONGODB_URI")

if not BOT_TOKEN or not MONGODB_URI:
    logger.error("❌ Не встановлені BOT_TOKEN або MONGODB_URI в змінних оточення")
    exit(1)

# Підключення до MongoDB Atlas
mongo_client = AsyncIOMotorClient(MONGODB_URI)
db = mongo_client.get_default_database()      # назва БД береться з URI
